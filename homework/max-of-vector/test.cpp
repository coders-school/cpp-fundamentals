...
TEST_P(MaxOfVectorTestFixture, ShouldFindMax) {
  ...
  EXPECT_EQ(maxOfVector(std::vector<int>(vec.begin(), vec.end())),
            expected_result);
}

INSTANTIATE_TEST_SUITE_P(
    MaxOfVectorTests, MaxOfVectorTestFixture,
    ::testing::Values(
        ...
        std::make_tuple(std::vector<int>{std::numeric_limits<int>::min(), -1},
                        -1),
        std::make_tuple(std::vector<int>{std::numeric_limits<int>::max(), -1},
        ...
        std::make_tuple(std::vector<int>{-1, 0, 1, -2, -4, -6, -7, -8, -8},
                        1)));
